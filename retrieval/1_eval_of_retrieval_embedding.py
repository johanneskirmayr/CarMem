...
def parse_args():
    ...
    parser.add_argument("--extraction_result_dir", type=str, default="extraction/evaluation/gpt4o/eval_of_extraction_in_schema/dataset/eval_of_extraction.jsonl")
    parser.add_argument("--trace_by_langsmith", type=bool, default=False)
    parser.add_argument("--langsmith_project_name", type=str, default="eval_of_retrieval_embedding")
    parser.add_argument("--embedding_type", type=str, default="text_attr_dc", help="The type of embedding to use for retrieval, either text_attr_dc or text")
    parser.add_argument("--output_dir_text_attr_dc", type=str, default="retrieval/evaluation/gpt4o/eval_of_rerieval_embedding/dataset/text_attr_dc/")
    parser.add_argument("--output_dir_text", type=str, default="retrieval/evaluation/gpt4o/eval_of_rerieval_embedding/dataset/text/")
    parser.add_argument("--output_file", type=str, default="eval_of_retrieval_embedding.jsonl")
    ...
    parser.add_argument("--collection_name_text_attr_dc", type=str, default="user_preferences_vctr_dc_attr_text")
    parser.add_argument("--collection_name_text", type=str, default="user_preferences_vctr_text")
...
def main():
    ...
    for line in extraction_for_eval_lines[:-1]: # last line is the evaluation scores
        ...
        for conversation in line["data"]:
            ...
            subcategory_pyd = category_to_pyd_category(input_string=subcategory, category="subcategory")
            ...
            total_number_same_subcategory_preferences += number_same_subcategory_preferences
            next_conversation_question = conversation["next_conversation_question"]
            next_conversation_question_embedding = get_embedding().embed_query(next_conversation_question)
            ...
            retrieved_ids = [retrieved_preference['id'] for retrieved_preference in retrieved_preferences[0]]
            
            ratio_relevant_irrelevant_ssc = [0, 0, 0]
            # top-(number_same_subcategory) 
            ground_truth_preference_retrieved_top_ssc = False
            for id, preference in zip(retrieved_ids[:number_same_subcategory_preferences], retrieved_preferences[0]):
            ...
            ratio_relevant_irrelevant_ssc[2] = ratio_relevant_irrelevant_ssc[0]/number_same_subcategory_preferences
            total_ratio_relevant_ssc.append(ratio_relevant_irrelevant_ssc[2])
            all_ground_truth_preference_retrieved_top_ssc.append(ground_truth_preference_retrieved_top_ssc)

            # top-(number_same_subcategory + 1)
            ratio_relevant_irrelevant_ssc_p1 = [0, 0, 0] 
            ground_truth_preference_retrieved_top_ssc_p1 = False
            for id, preference in zip(retrieved_ids[:number_same_subcategory_preferences+1], retrieved_preferences[0]):
                if preference['entity']['subcategory'] == subcategory_pyd:
            ...
            ratio_relevant_irrelevant_ssc[2] = ratio_relevant_irrelevant_ssc_p1[0]/(number_same_subcategory_preferences + 1)
            total_ratio_relevant_ssc_p1.append(ratio_relevant_irrelevant_ssc[2])
            all_ground_truth_preference_retrieved_top_ssc_p1.append(ground_truth_preference_retrieved_top_ssc_p1)

            # top-(number_same_subcategory + 2) 
            ratio_relevant_irrelevant_ssc_p2 = [0, 0, 0] 
            ground_truth_preference_retrieved_top_ssc_p2 = False
            for id, preference in zip(retrieved_ids[:number_same_subcategory_preferences+2], retrieved_preferences[0]):
                if preference['entity']['subcategory'] == subcategory_pyd:
            ...
            ratio_relevant_irrelevant_ssc[2] = ratio_relevant_irrelevant_ssc_p2[0]/(number_same_subcategory_preferences + 2)
            total_ratio_relevant_ssc_p2.append(ratio_relevant_irrelevant_ssc[2])
            all_ground_truth_preference_retrieved_top_ssc_p2.append(ground_truth_preference_retrieved_top_ssc_p2)
            # remove vectors
            ...
            conversation["evaluation_next_conversation_question_embedding"] = {
                ...
                "retrieved_preferences_top_ssc_p2": retrieved_preferences[0][:number_same_subcategory_preferences+2],
    ...
    accuracy_ground_truth_preference_retrieved_top_ssc = sum(all_ground_truth_preference_retrieved_top_ssc)/len(all_ground_truth_preference_retrieved_top_ssc)
    accuracy_ground_truth_preference_retrieved_top_ssc_p1 = sum(all_ground_truth_preference_retrieved_top_ssc_p1)/len(all_ground_truth_preference_retrieved_top_ssc_p1)
    accuracy_ground_truth_preference_retrieved_top_ssc_p2 = sum(all_ground_truth_preference_retrieved_top_ssc_p2)/len(all_ground_truth_preference_retrieved_top_ssc_p2)
    mean_ratio_relevant_ssc = sum(total_ratio_relevant_ssc)/len(total_ratio_relevant_ssc)
    mean_ratio_relevant_ssc_p1 = sum(total_ratio_relevant_ssc_p1)/len(total_ratio_relevant_ssc_p1)
    mean_ratio_relevant_ssc_p2 = sum(total_ratio_relevant_ssc_p2)/len(total_ratio_relevant_ssc_p2)
    all_evaluation_scores = {
        ...
        "mean_number_same_subcategory_preferences": total_number_same_subcategory_preferences/total_number_questions,
        "mean_questions_per_user": total_number_questions/len(extraction_for_eval_lines),
