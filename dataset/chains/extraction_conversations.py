...
from prompts_database.extraction_conversation_prompts_dynamic_v3 import (
    EXTRACTION_CONVERSATION_PROMPT,
    EXTRACTION_CONVERSATION_PROMPT_NO_PREFERENCE,
)
...
class ExtractionConversationChain:
    ...
    def __init__(self):
        self.generator = EXTRACTION_CONVERSATION_PROMPT | get_llm_gpt4o(
            temperature=0.7
        ).bind(response_format={"type": "json_object"})
        self.generator_no_preference = (
            EXTRACTION_CONVERSATION_PROMPT_NO_PREFERENCE
            | get_llm_gpt4o(temperature=0.7).bind(
    ...
    def sample_dynamic_prompt_inputs(self, random_seed):
        random.seed(
            random_seed
        )  # below dynamic pieces should be random but fixed within a user
        self.user_profile = {
            "Age": "Placeholder",
        ...
        self.user_profile["Technological_Proficiency"] = random.choice(
            self.user_profile["Technological_Proficiency"]
        )
        ...
        level_of_assistant_proactivity = ["medium", "high", "very high - no questions"]
        self.level_of_proactivity_assistant = random.choice(
        ...
        self.preference_strength_modulation = random.choice(
            preference_strength_modulation
        )

        self.conversation_length = random.randrange(
            start=2, stop=10, step=2
        )  # can only be even
        # self.conversation_length = 4

        self.position_user_preference_in_conv = random.randrange(
            start=1, stop=self.conversation_length, step=2
        )  # can only be odd

        return

    def generate_one_conversation(
        self, topic: str, user_preference: str, random_seed=None
    ):
        self.sample_dynamic_prompt_inputs(random_seed=random_seed)
        self.few_shot_example = sample_few_shot_example(
            category=topic.split("; ")[-1], random_seed=random_seed
        )
        ...
        conversation = self.generator.invoke(
            {
                ...
                "level_of_proactivity_assistant": str(
                    self.level_of_proactivity_assistant
                ),
                "preference_strength_modulation": str(
                    self.preference_strength_modulation
                ),
                "conversation_length": str(self.conversation_length),
                "position_user_preference_in_conv": str(
                    self.position_user_preference_in_conv
                ),
    ...
    def generate_one_conversation_no_preference(
        self,
        topic: str,
        user_preference: str,
        random_seed=None,
        user_profile=None,
        user_conversation_style=None,
        car_location_city=None,
        level_of_proactivity_assistant=None,
        preference_strength_modulation=None,
        conversation_length=None,
        position_user_preference_in_conv=None,
        ...
        user_conversation_style = (
            user_conversation_style or self.user_conversation_style
        )
        car_location_city = car_location_city or self.car_location_city
        level_of_proactivity_assistant = (
            level_of_proactivity_assistant or self.level_of_proactivity_assistant
        )
        preference_strength_modulation = (
            preference_strength_modulation or self.preference_strength_modulation
        )
        conversation_length = conversation_length or self.conversation_length
        position_user_preference_in_conv = (
            position_user_preference_in_conv or self.position_user_preference_in_conv
        )
