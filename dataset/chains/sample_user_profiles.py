...
def sample_user_profiles(path_to_categories: str, number_user_profiles: int, number_attributes_per_user: int, random_seed: int = None):
    """
    ...
    for _ in range(number_user_profiles):
        ...
        fuel_type = random.choice(['Gas Station Preferences', 'Charging Station Preferences'])
        ...
        df['Weight'] = df['Detail Category'].transform(lambda x: 1 / (df['Detail Category'].value_counts()[x] * unique_categories_count))

        sampled_df = df.sample(n=number_attributes_per_user, replace=False, weights='Weight', random_state=random_seed).sort_index()
        for idx, row in sampled_df[sampled_df['Preference Type']=='MNP'].iterrows():
            sampled_df.loc[idx, 'Attributes'] = random.choice(row['Attributes'].split(", "))
...
def main():
    test_user_profiles, test_sampled_indices, test_sampled_categories = sample_user_profiles('dataset/categories_v4.csv', 10, 10)
    ...
    axs[0].bar(test_sampled_indices.keys(), test_sampled_indices.values(), color='orange', edgecolor='black')
    ...
    axs[1].bar(test_sampled_categories.keys(), test_sampled_categories.values(), color='skyblue', edgecolor='black')
    axs[1].set_xlabel('Detail Category')
    axs[1].set_ylabel('Frequency')
    axs[1].set_title('Histogram of Sampled Values per Unique \'Detail Category\' (100 iterations)')
