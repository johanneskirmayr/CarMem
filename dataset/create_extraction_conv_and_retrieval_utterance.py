...
def parse_args():
    ...
    parser.add_argument("--user_profiles_dir", type=str, default="dataset/userprofiles/user_profiles.jsonl", 
    ...
    parser.add_argument("--langsmith_project_name", type=str, default="create_incar_conv_and_retrieval_utterance")
...
def main():
    ...
    for idx, user_profile in tqdm(enumerate(user_profiles), total=len(user_profiles)):
        ...
        for _, row in user_profile_df.iterrows():
            ...
            user_preference = row["Main Category"] + "; " + row["Subcategory"] + "; " + row["Detail Category"] + "; " + row["Attributes"]
            ...
            conversation, metadata_prompt = extraction_conversation_chain.generate_one_conversation(topic=topic, user_preference=user_preference, random_seed=idx) # random_seed so that per user the dynamic pieces are equal
            ...
            next_conversation_questions, metadata_prompt_question = next_conversation_questions_chain.generate_next_conversation_questions(topic=topic, user_preference=user_preference, conversation=conversation_list, user_conversation_style = metadata_prompt["user_conversation_style"])
            next_conversation_questions_dict = json.loads(next_conversation_questions.content)
            print(next_conversation_questions_dict)

            # Parse output to pandas dataframe
            flattened_data = [(speaker, text) for turn in conversation_list for speaker, text in turn.items()]
            ...
            label_studio_task = {
                "data": {
                    "messages": [
                        {"role": row['speaker'], "content": row['text']} for _, row in df_conversation.iterrows()
                    ],
            ...
            if args.print_pretty:
                # Write to file
                os.makedirs(f"dataset/generated_unlabeled_conversations/{run}/", exist_ok=True)
                with open(f"dataset/generated_unlabeled_conversations/{run}/conversations.jsonl", 'a', encoding='utf-8') as file:
                    file.write(json.dumps(label_studio_task, ensure_ascii=False) + "\n")
                    # file.flush()
                # Print to file pretty
                with open(f"dataset/generated_unlabeled_conversations/{run}/conversations_print_pretty.txt", 'a') as file:
                    if idx==counter:
                    ...
                    for sentence in label_studio_task["data"]["messages"]:
                        pretty_string += f"{sentence['role']}: " + f"{sentence['content']}" + "\n"
                    pretty_string += "\n"

                    pretty_string += f"Next Conversation Questions: \n"
                    pretty_string += "Next conversation question: " + str(label_studio_task["data"]["next_conversation_questions"]["next_conversation_question_user"]) + "\n"
                    ...
                    pretty_string += "user_uuid: " + str(label_studio_task["data"]["meta_info"]["user_uuid"]) + "\n"
                    pretty_string += "user_profile: " + str(label_studio_task["data"]["meta_info"]["user_profile"]) + "\n"
                    pretty_string += "user_conversation_style: " + str(label_studio_task["data"]["meta_info"]["user_conversation_style"]) + "\n"
                    pretty_string += "car_location_city: " + str(label_studio_task["data"]["meta_info"]["car_location_city"]) + "\n"
                    pretty_string += "level_of_proactivity_assistant: " + str(label_studio_task["data"]["meta_info"]["level_of_proactivity_assistant"]) + "\n"
                    pretty_string += "preference_strength_modulation: " + str(label_studio_task["data"]["meta_info"]["preference_strength_modulation"]) + "\n"
                    pretty_string += "conversation_length: " + str(label_studio_task["data"]["meta_info"]["conversation_length"]) + "\n"
                    pretty_string += "position_user_preference_in_conv: " + str(label_studio_task["data"]["meta_info"]["position_user_preference_in_conv"]) + "\n"
                    pretty_string += "few_shot_example: " + str(label_studio_task["data"]["meta_info"]["few_shot_examples"]) + "\n"
                    pretty_string += "\n"
            ...
            if args.send_to_dataset:
                ...
                datapoint = {
                    ...
                    "next_conversation_question": next_conversation_questions_dict["next_conversation_question_user"], 
                    "meta_info": {
                        "level_of_proactivity_assistant": metadata_prompt["level_of_proactivity_assistant"],
                        "preference_strength_modulation": metadata_prompt["preference_strength_modulation"],
                        "conversation_length": metadata_prompt["conversation_length"],
                        "position_user_preference_in_conv": metadata_prompt["position_user_preference_in_conv"],
