...
def parse_args():
    ...
    parser.add_argument(
        "--langsmith_project_name", type=str, default="create_maintenance_utterance"
    )
...
def main():
    ...
    for idx, train_dataset_line in enumerate(track(dataset_lines)):
        for conversation_data in tqdm(train_dataset_line["data"]):
            ...
            same_category_attributes = df_categories.loc[
                df_categories["Detail Category"] == detail_category, "Attributes"
            ]
            same_category_attributes = [
                attr.strip() for attr in same_category_attributes.iloc[0].split(",")
            ]
            ...
            if same_category_attributes:
                ...
                detail_category == "Need for Handicapped Accessible Parking"
            ):  # this is the only category where there is only one attribute
                different_attribute = "No"
            else:
                raise ValueError

            different_preference = ";".join(
                conversation_data["user_preference"].split(";")[:-1]
                + [" " + different_attribute]
            )

            # sample_different_attribute =
            maintenance_questions, metadata = (
                maintenance_question_chain.generate_maintenance_questions(
                    ...
                    user_conversation_style=train_dataset_line[
            ...
            conversation_data["maintenance_questions"][
                "different_attribute"
            ] = different_attribute
