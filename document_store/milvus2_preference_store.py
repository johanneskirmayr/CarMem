...
class Milvus2PreferenceStore():
    ...
    def _create_collection_and_index(
        ...
        if not has_collection:
            fields = [
                FieldSchema(name="pk", dtype=DataType.VARCHAR, max_length=500, is_primary=True, auto_id=False), # "pk" is langchain convention
                FieldSchema(name="vector", dtype=DataType.FLOAT_VECTOR, dim=self.embedding_dim), # "vector" is langchain convention
                FieldSchema(name="text", dtype=DataType.VARCHAR, max_length=50000), # "text" is langchain convention
                FieldSchema(name="main_category", dtype=DataType.VARCHAR, max_length=50000),
                FieldSchema(name="subcategory", dtype=DataType.VARCHAR, max_length=50000),
                FieldSchema(name="detail_category", dtype=DataType.VARCHAR, max_length=50000),
                FieldSchema(name="attribute", dtype=DataType.VARCHAR, max_length=50000),
                FieldSchema(name='user_name', dtype=DataType.VARCHAR, description="", max_length=512, is_partition_key=True),
            ]

            collection_schema = CollectionSchema(fields=fields, enable_dynamic_field=True, partition_key_field = "user_name")
            collection = Collection(name=collection_name, schema=collection_schema)
        else:
            logger.warning(f"Collection {collection_name} already exists. Will not be recreated, please delete or rename.")
        ...
        if not has_index:
            collection.create_index(
                field_name="vector",
                index_params={"index_type": self.index_type, "metric_type": self.metric_type, "params": self.index_param},
            )
