...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="extraction/evaluation/gpt4o/extraction_for_eval_in_schema/dataset/extraction_for_eval.jsonl",
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="extraction/evaluation/gpt4o/eval_of_extraction_in_schema/dataset/",
    )
...
def main():
    ...
    for line in extraction_for_eval_lines:
        for conversation in line["data"]:
            ...
            label_main_pred = [
                ...
                for idx in range(
                    conversation_extraction["number_preferences_extracted"]
                )
            ]
            label_main_category[1].extend([label_main_pred])

            conv_main_accuracy = accuracy_score(
                y_true=mlb_main.fit_transform([label_main_true]),
                y_pred=mlb_main.transform([label_main_pred]),
            )
            conv_main_prec_rec_f_sup = precision_recall_fscore_support(
                y_true=mlb_main.fit_transform([label_main_true]),
                y_pred=mlb_main.transform([label_main_pred]),
                average="micro",
            )
            ...
            conv_sub_accuracy = accuracy_score(
                y_true=mlb_sub.fit_transform([label_sub_true]),
                y_pred=mlb_sub.transform([label_sub_pred]),
            )
            conv_sub_prec_rec_f_sup = precision_recall_fscore_support(
                y_true=mlb_sub.fit_transform([label_sub_true]),
                y_pred=mlb_sub.transform([label_sub_pred]),
                average="micro",
            )
            ...
            conv_detail_accuracy = accuracy_score(
                y_true=mlb_detail.fit_transform([label_detail_true]),
                y_pred=mlb_detail.transform([label_detail_pred]),
            )
            conv_detail_prec_rec_f_sup = precision_recall_fscore_support(
                y_true=mlb_detail.fit_transform([label_detail_true]),
                y_pred=mlb_detail.transform([label_detail_pred]),
                average="micro",
            )
            ...
            if (
                len(label_main_pred) == 1
                and len(label_sub_pred) == 1
                and len(label_detail_pred) == 1
            ):
            ...
            label_main_category_true_repeated[0].extend(
                [label_main_true * len(label_main_pred)]
            )
            label_main_category_true_repeated[1].extend([label_main_pred])
            label_subcategory_true_repeated[0].extend(
                [label_sub_true * len(label_sub_pred)]
            )
            label_subcategory_true_repeated[1].extend([label_sub_pred])
            label_detail_category_true_repeated[0].extend(
                [label_detail_true * len(label_detail_pred)]
            )
            label_detail_category_true_repeated[1].extend([label_detail_pred])
            # =======

            # ==== check if ground truth got extracted based on categories ====
            ground_truth_preference_extracted = (
                True
                if conversation_extraction["ground_truth_preference_categories_labels"][
                    2
                ]
                in [
                    conversation_extraction[
                        f"extracted_preference_{idx}_categories_label"
                    ][2]
                    for idx in range(
                        conversation_extraction["number_preferences_extracted"]
                    )
            ...
            for idx in range(conversation_extraction["number_preferences_extracted"]):
                extracted_label = conversation_extraction[
                    f"extracted_preference_{idx}_categories_label"
                ][2]
                if (
                    conversation_extraction[
                        "ground_truth_preference_categories_labels"
                    ][2]
                    == extracted_label
                ):
            ...
            if not ground_truth_preference_extracted:
                ground_truth_labels = conversation_extraction[
                    "ground_truth_preference_categories_labels"
                ]
                extracted_labels = [
                    conversation_extraction[
                ...
                log_info(
                    f"Ground truth label: {ground_truth_labels}, Extracted labels: {extracted_labels}"
                )
            # ========
            ...
            if ground_truth_preference_extracted:
                concat_conversation = " ".join(
                    [
                        str(list(turn.values())[0])
                        for turn in conversation["extraction_conversation"]
                    ]
                )
                concat_conversation_user_only = " ".join(
                    [
                        str(list(turn.values())[0])
                        for turn in conversation["extraction_conversation"][::2]
                    ]
                )
                ...
                if preference.lower() in concat_conversation.lower():
                    ...
                    if (
                        preference.lower()
                        in conversation_extraction[
                            f"extracted_preference_{found_index}_full"
                        ]["text"].lower()
                    ):
    ...
    all_main_accuracy = accuracy_score(
        y_true=mlb_main.transform(label_main_category[0]),
        y_pred=mlb_main.transform(label_main_category[1]),
    )
    all_main_prec_rec_f_sup = precision_recall_fscore_support(
        y_true=mlb_main.transform(label_main_category[0]),
        y_pred=mlb_main.transform(label_main_category[1]),
        average="micro",
    )
    ...
    all_sub_accuracy = accuracy_score(
        y_true=mlb_sub.transform(label_subcategory[0]),
        y_pred=mlb_sub.transform(label_subcategory[1]),
    )
    all_sub_prec_rec_f_sup = precision_recall_fscore_support(
        y_true=mlb_sub.transform(label_subcategory[0]),
        y_pred=mlb_sub.transform(label_subcategory[1]),
        average="micro",
    )
    ...
    all_detail_accuracy = accuracy_score(
        y_true=mlb_detail.transform(label_detail_category[0]),
        y_pred=mlb_detail.transform(label_detail_category[1]),
    )
    all_detail_prec_rec_f_sup = precision_recall_fscore_support(
        y_true=mlb_detail.transform(label_detail_category[0]),
        y_pred=mlb_detail.transform(label_detail_category[1]),
        average="micro",
    )
    ...
    all_main_accuracy_one_pref_only = accuracy_score(
        y_true=mlb_main.transform(label_main_category_one_pref_only[0]),
        y_pred=mlb_main.transform(label_main_category_one_pref_only[1]),
    )

    all_sub_accuracy_one_pref_only = accuracy_score(
        y_true=mlb_sub.transform(label_subcategory_one_pref_only[0]),
        y_pred=mlb_sub.transform(label_subcategory_one_pref_only[1]),
    )

    # print("binarized_labels_example: \n", label_subcategory[0], label_subcategory[1])
    # print("binarized_labels_example: \n", mlb_sub.transform(label_subcategory[0]), mlb_sub.transform(label_subcategory[1]))

    all_detail_accuracy_one_pref_only = accuracy_score(
        y_true=mlb_detail.transform(label_detail_category_one_pref_only[0]),
        y_pred=mlb_detail.transform(label_detail_category_one_pref_only[1]),
    )
    # ========
    ...
    if args.confusion_matrix:
        label_main_category_one_pref_only_true = [
            item for sublist in label_main_category_one_pref_only[0] for item in sublist
        ]
        label_main_category_one_pref_only_pred = [
            item for sublist in label_main_category_one_pref_only[1] for item in sublist
        ]
        label_subcategory_one_pref_only_true = [
            item for sublist in label_subcategory_one_pref_only[0] for item in sublist
        ]
        label_subcategory_one_pref_only_pred = [
            item for sublist in label_subcategory_one_pref_only[1] for item in sublist
        ]
        label_detail_category_one_pref_only_true = [
            item
            for sublist in label_detail_category_one_pref_only[0]
            for item in sublist
        ]
        label_detail_category_one_pref_only_pred = [
            item
            for sublist in label_detail_category_one_pref_only[1]
            for item in sublist
        ]
        confusion_matrix_main = confusion_matrix(
            y_true=label_main_category_one_pref_only_true,
            y_pred=label_main_category_one_pref_only_pred,
            labels=mlb_main.classes_,
        )
        confusion_matrix_sub = confusion_matrix(
            y_true=label_subcategory_one_pref_only_true,
            y_pred=label_subcategory_one_pref_only_pred,
            labels=mlb_sub.classes_,
        )
        confusion_matrix_detail = confusion_matrix(
            y_true=label_detail_category_one_pref_only_true,
            y_pred=label_detail_category_one_pref_only_pred,
            labels=mlb_detail.classes_,
        )
        confusion_matrix_main_nm = confusion_matrix(
            normalize="true",
            y_true=label_main_category_one_pref_only_true,
            y_pred=label_main_category_one_pref_only_pred,
            labels=mlb_main.classes_,
        )
        confusion_matrix_sub_nm = confusion_matrix(
            normalize="true",
            y_true=label_subcategory_one_pref_only_true,
            y_pred=label_subcategory_one_pref_only_pred,
            labels=mlb_sub.classes_,
        )
        confusion_matrix_detail_nm = confusion_matrix(
            normalize="true",
            y_true=label_detail_category_one_pref_only_true,
            y_pred=label_detail_category_one_pref_only_pred,
            labels=mlb_detail.classes_,
        ...
        all_evaluation_scores["confusion_matrix_detail"] = (
            confusion_matrix_detail.tolist()
        )
        ...
        cm = ConfusionMatrixDisplay(
            confusion_matrix_main_nm, display_labels=display_labels_main
        )
        ...
        plt.title(
            "Normalized Confusion Matrix for Main Category (1 pref. extracted)",
            fontdict=title_font,
        )
        plt.tight_layout()
        ...
        fig, ax = plt.subplots(figsize=(10, 7), sharex=True, sharey=True, squeeze=True)
        cm = ConfusionMatrixDisplay(
            confusion_matrix_sub_nm, display_labels=display_labels_sub
        )
        cm.plot(ax=ax, xticks_rotation="vertical")
        for text in ax.texts:
            if text.get_text().startswith("0."):
        ...
        ax.tick_params(axis="both", which="major", labelsize=14)
        plt.title(
            "Normalized Confusion Matrix for Subcategory (1 pref. extracted)",
            fontdict=title_font,
        )
        plt.tight_layout()
        ...
        cm = ConfusionMatrixDisplay(
            confusion_matrix_detail_nm, display_labels=display_labels_detail
        )
        ...
        for text in ax.texts:
            if text.get_text().startswith("0."):
                text.set_text(text.get_text()[1:])
        plt.title(
            "Normalized Confusion Matrix for Detail Category (1 pref. extracted)",
            fontdict=title_font,
        )
        plt.tight_layout()
        ...
        fig, ax = plt.subplots(figsize=(10, 7), sharex=True, sharey=True, squeeze=True)
        cm = ConfusionMatrixDisplay(
            confusion_matrix_main, display_labels=display_labels_main
        )
        cm.plot(ax=ax, xticks_rotation="vertical")
        ax.set_xlabel("Predicted Preference Category", fontdict=label_font)
        ax.set_ylabel("Predicted Preference Category", fontdict=label_font)
        ...
        for text in ax.texts:
            if text.get_text().startswith("0."):
                text.set_text(text.get_text()[1:])
        plt.title(
            "Confusion Matrix for Main Category (1 pref. extracted)",
            fontdict=title_font,
        )
        plt.tight_layout()
        ...
        fig, ax = plt.subplots(figsize=(10, 7), sharex=True, sharey=True, squeeze=True)
        cm = ConfusionMatrixDisplay(
            confusion_matrix_sub, display_labels=display_labels_sub
        )
        cm.plot(ax=ax, xticks_rotation="vertical")
        ax.set_xlabel("Predicted Preference Category", fontdict=label_font)
        ax.set_ylabel("Predicted Preference Category", fontdict=label_font)
        ...
        for text in ax.texts:
            if text.get_text().startswith("0."):
                text.set_text(text.get_text()[1:])
        plt.title(
            "Confusion Matrix for Subcategory (1 pref. extracted)", fontdict=title_font
        )
        plt.tight_layout()
        ...
        fig, ax = plt.subplots(figsize=(20, 14), sharex=True, sharey=True, squeeze=True)
        cm = ConfusionMatrixDisplay(
            confusion_matrix_detail, display_labels=display_labels_detail
        )
        cm.plot(ax=ax, xticks_rotation="vertical")
        ax.set_xlabel("Predicted Preference Category", fontdict=label_font)
        ax.set_ylabel("Predicted Preference Category", fontdict=label_font)
        ...
        for text in ax.texts:
            if text.get_text().startswith("0."):
                text.set_text(text.get_text()[1:])
        plt.title(
            "Confusion Matrix for Detail Category (1 pref. extracted)",
            fontdict=title_font,
        )
        plt.tight_layout()
        ...
        for text in ax.texts:
            if text.get_text().startswith("0."):
                text.set_text(text.get_text()[1:])
        plt.title(
            "Normalized Multi-Label Confusion Matrix for Main Category",
            fontdict=title_font,
        )
        plt.tight_layout()
        ...
        for text in ax.texts:
            if text.get_text().startswith("0."):
                text.set_text(text.get_text()[1:])
        plt.title(
            "Normalized Multi-Label Confusion Matrix for Sub Category",
            fontdict=title_font,
        )
        plt.tight_layout()
        ...
        for text in ax.texts:
            if text.get_text().startswith("0."):
                text.set_text(text.get_text()[1:])
        plt.title(
            "Normalized Multi-Label Confusion Matrix for Detail Category",
            fontdict=title_font,
        )
        plt.tight_layout()
        ...
        for text in ax.texts:
            if text.get_text().startswith("0."):
                text.set_text(text.get_text()[1:])
        plt.title(
            "Multi-Label Confusion Matrix for Detail Category", fontdict=title_font
        )
        plt.tight_layout()
