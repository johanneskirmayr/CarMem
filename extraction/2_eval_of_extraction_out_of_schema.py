...
def parse_args():
    ...
    parser.add_argument("--extraction_for_eval_data_dir", type=str, default="extraction/evaluation/gpt4o/extraction_for_eval_out_of_schema/dataset/extraction_for_eval.jsonl")
    ...
    parser.add_argument("--output_dir", type=str, default="extraction/evaluation/gpt4o/eval_of_extraction_out_of_schema/dataset/")
...
def main():
    ...
    label_main_category_empty, label_subcategory_empty, label_detail_category_empty = ([], []), ([], []), ([], []) # ([[ground_truth_label_conv1], ...], [[predicted_labels_conv1], ...])
    ...
    # for line in extraction_for_eval_lines:
        for conversation in line["data"]:
            ...
            number_preferences_valid_extracted = conversation_extraction["number_preferences_extracted"]
            # main_category
            ...
            label_main_category_empty[0].extend([[]]) # empty because we don't want to extract the ground truth

            label_main_pred = [conversation_extraction[f"extracted_preference_{idx}_categories_label"][0] for idx in range(number_preferences_valid_extracted)]
            label_main_category[1].extend([label_main_pred])


            conv_main_accuracy = accuracy_score(y_true=mlb_main.fit_transform([label_main_true_empty]), y_pred=mlb_main.transform([label_main_pred]))
            ...
            label_sub_pred = [conversation_extraction[f"extracted_preference_{idx}_categories_label"][1] for idx in range(number_preferences_valid_extracted)]
            label_subcategory[1].extend([label_sub_pred])


            conv_sub_accuracy = accuracy_score(y_true=mlb_sub.fit_transform([label_sub_true_empty]), y_pred=mlb_sub.transform([label_sub_pred]))
            ...
            label_detail_pred = [conversation_extraction[f"extracted_preference_{idx}_categories_label"][2] for idx in range(number_preferences_valid_extracted)]
            ...
            conv_detail_accuracy = accuracy_score(y_true=mlb_detail.fit_transform([label_detail_true_empty]), y_pred=mlb_detail.transform([label_detail_pred]))

            number_preferences_extracted = conversation_extraction["number_preferences_extracted"]
            ...
            if "number_other_extracted" in conversation_extraction:
                number_preferences_extracted_other = conversation_extraction["number_other_extracted"]
                number_preferences_extracted_other_total += number_preferences_extracted_other
                conversation_extraction["evaluation"].update({"number_other_extracted": number_preferences_extracted_other})
            counter+=1
    ...
    all_main_accuracy = accuracy_score(y_true=mlb_main.transform(label_main_category_empty[0]), y_pred=mlb_main.transform(label_main_category[1]))

    all_sub_accuracy = accuracy_score(y_true=mlb_sub.transform(label_subcategory_empty[0]), y_pred=mlb_sub.transform(label_subcategory[1]))
    ...
    all_detail_accuracy = accuracy_score(y_true=mlb_detail.transform(label_detail_category_empty[0]), y_pred=mlb_detail.transform(label_detail_category[1]))
    # ========
