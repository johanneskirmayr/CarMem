...
import os
import uuid
from typing import Any, AsyncGenerator, Dict, Iterable, List, Optional, Sequence, Union, cast
...
from utils.custom_logger import log_debug, log_info, log_error
...
def create_preference_extraction_chain_pydantic(
    ...
    function = cast(Dict, convert_pydantic_to_openai_function(
        model=pydantic_schema,
        name=data["schema"]["name"],
        description=data["schema"]["description"]
        ))
    extraction_prompt = prompt if type(prompt)==ChatPromptTemplate else ChatPromptTemplate.from_template(prompt)
    ...
    chain = (
        extraction_prompt
        | llm.bind(function_call={"name": data["schema"]["name"]}, functions=[function])
    )
    return chain
...
class PreferenceMemory():
    ...
    def retry_chain_with_validation_error(self, extraction_output: str, validation_error: str, chain: Chain) -> Chain:
        chain_ = chain.copy()
        original_prompt = chain.first.messages[0].prompt.template
        chain_.first.messages[0].prompt.template = chain.first.messages[0].prompt.template + "\n" + "\n# Errors from previous try:\n Your previous call did not produce a valid output format (possible reasons: category was skipped, non-existing key, subcategory corresponds to different parent category): " + str(extraction_output).replace("{", "{{").replace("}", "}}") + ". This failed because of the validation error:\n" + str(validation_error).replace("{", "{{").replace("}", "}}") + "Please correct the error and extract the same preference in the correct format."
        return chain_, original_prompt
    
    def filter_none_values(self, d):
        if isinstance(d, dict):
            return {k: self.filter_none_values(v) for k, v in d.items() if v is not None}
        elif isinstance(d, list):
            return [self.filter_none_values(v) for v in d if v is not None]
        else:
            return d
        
    def validate_output_and_retry(self, output_extraction, pydantic_schema, chain, username, messages_string):
        output_extraction_arguments = json.loads(output_extraction.additional_kwargs['function_call']['arguments'])
        validation_error = self.validate_extraction(output_extraction_arguments, pydantic_schema)
        
        if validation_error:
            extraction_chain_retry, original_prompt = self.retry_chain_with_validation_error(extraction_output=str(output_extraction_arguments), validation_error=validation_error, chain=chain)
            output_extraction = extraction_chain_retry.invoke(input={"user_name": username, "conversation": messages_string})
            
            # remove validation error message again
            chain.first.messages[0].prompt.template = original_prompt
            
            output_extraction_arguments = json.loads(output_extraction.additional_kwargs['function_call']['arguments'])
            validation_error = self.validate_extraction(extraction_result=output_extraction_arguments, pydantic_schema=pydantic_schema)
            if validation_error:
                valid_at_try = None
                log_error(f"Repeating Validation of the extraction output w.r.t the pydantic schema: {validation_error}")
                return output_extraction, valid_at_try
            else:
                valid_at_try = 2
                # filter None outputs
                filtered_extraction = json.dumps(self.filter_none_values(json.loads(output_extraction.additional_kwargs['function_call']['arguments'])))
                output_extraction.additional_kwargs['function_call']['arguments'] = filtered_extraction
                return output_extraction, valid_at_try
        else:
            valid_at_try = 1
            # filter None outputs
