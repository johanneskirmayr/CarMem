...
def parse_args():
    ...
    parser.add_argument("--langsmith_project_name", type=str, default="extraction_negate_preference")
    parser.add_argument("--output_dir", type=str, default="maintenance/evaluation/gpt4o/extraction_maintenance_utterances/dataset/")
    parser.add_argument("--output_file", type=str, default="extraction_maintenance_utterances_test.jsonl")
...
async def main():
    ...
    for idx, line in enumerate(track(train_dataset_lines[:-1])): # one user in for loop, exclude line with overall results
        ...
        for conversation in tqdm(line["data"]): # one conversation in for loop
            ...
            preference_equal.update({
                "main_category": category_to_pyd_category(datapoint_user_preference_dict["main_category"], "main_category"),
                "subcategory": category_to_pyd_category(datapoint_user_preference_dict["subcategory"], "subcategory"),
                "detail_category": category_to_pyd_category(datapoint_user_preference_dict["detail_category"], "detail_category"),
                "text": maintenance_questions['question_equal_preference'],
                "user_name": str(john_user_id)
            })
            preference_equal_categories_eval = {key: preference_equal[key] for key in keys_to_include_category_eval if key in preference_equal}
            preference_equal_dict = {
                "extracted_preference_equal_full": preference_equal,
                "extracted_preference_equal_categories_labels": convert_preference_to_labels(preference_equal_categories_eval)
            }
            ...
            preference_different_categories_eval = {key: preference_different[key] for key in keys_to_include_category_eval if key in preference_different}
            preference_different_dict = {
                "extracted_preference_different_full": preference_different,
                "extracted_preference_different_categories_labels": convert_preference_to_labels(preference_different_categories_eval)
            }

            # perform extraction for negate preference
            preference_negate_dict = {}
            with get_openai_callback() as cb:
                output_extraction_maintenance_negate = extraction_chain.invoke(input={"user_name": john_username, "conversation": question_negate})
            output_extraction_maintenance_negate_retry, valid_at_try = preference_memory.validate_output_and_retry(output_extraction=output_extraction_maintenance_negate, pydantic_schema=PreferencesFunctionOutput, chain=extraction_chain, messages_string=question_negate, username=john_username)
            ...
            if valid_at_try==1:
                ...
                preference_negate_dict.update({
                    "failed_extraction_1": json.loads(output_extraction_maintenance_negate.additional_kwargs['function_call']['arguments']),
                })
                output_extraction_maintenance_negate = output_extraction_maintenance_negate_retry
            else:
                preference_negate_dict.update({
                    "failed_extraction_1": json.loads(output_extraction_maintenance_negate.additional_kwargs['function_call']['arguments']),
                    "failed_extraction_2": json.loads(output_extraction_maintenance_negate_retry.additional_kwargs['function_call']['arguments']),
                })

            function_json_extraction_maintenance_negate = json.loads(output_extraction_maintenance_negate.additional_kwargs['function_call']['arguments'])

            num_preference_counter = 0
            
            if function_json_extraction_maintenance_negate and not valid_at_try==None:
                for main_category, rest in function_json_extraction_maintenance_negate.items(): # in for loop are preferences of one main category
                                ...
                                if not (detail_category=="no_or_other_preferences"):
                                    ...
                                    for preference in value:
                                        ...
                                        preference_negate_dict.update({
                                            ...
                                            f"extracted_preference_negate_{num_preference_counter}_categories_label": convert_preference_to_labels(extracted_preference_categories_eval)
                                            })
            ...
            conversation["maintenance_questions"]["question_equal_extraction"] = preference_equal_dict
            conversation["maintenance_questions"]["question_negate_extraction"] = preference_negate_dict
            conversation["maintenance_questions"]["question_different_extraction"] = preference_different_dict

        # write line extended with extraction results, filter conversations where no extraction is performed
        filtered_data = filter(lambda d: "question_equal_extraction" in d["maintenance_questions"], line["data"])
