...
def parse_args():
    ...
    parser.add_argument(
        "--extraction_maintenance_result_dir",
        type=str,
        default="maintenance/evaluation/gpt4o/extraction_maintenance_utterances/dataset/extraction_maintenance_utterances.jsonl",
    )
    parser.add_argument("--trace_by_langsmith", type=bool, default=False)
    parser.add_argument(
        "--langsmith_project_name", type=str, default="call_maintenance_function"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="maintenance/evaluation/gpt4o/call_maintenance_function_for_eval/dataset/",
    )
    parser.add_argument(
        "--output_file", type=str, default="call_maintenance_function.jsonl"
    )
    parser.add_argument(
        "--perform_function",
        type=bool,
        default=False,
        help="wether to actually perform the function in the database or just simulate",
    )
...
def main():
    ...
    for line in track(train_dataset_lines):
        for conversation_data in tqdm(line["data"]):
            ...
            num_preference_counter = maintenance_questions[
            ...
            if num_preference_counter == 1:
                preference_labels = maintenance_questions["question_negate_extraction"][
                    f"extracted_preference_negate_0_categories_label"
                ]
                ...
                log_debug(
                    f"Either no or multiple preferences extracted from negate question"
                )
                preference_negate = {}
            ...
            for idx, extracted_preference in enumerate(dict_list):
                ...
                if extracted_preference:
                    ...
                    if not existing_preferences:
                        ...
                        if (
                            detail_category_to_type(
                                extracted_preference["detail_category"]
                            )
                            == "MP"
                        ):
                            print("\n Category Type 'MP': performing filter...")
                            with get_openai_callback() as cb:
                                run_tool_answer, tool_call = (
                                    maintenance.filter_extracted_preference_mp(
                                        incoming_preference=extracted_preference,
                                        existing_preferences=existing_preferences,
                                        perform_function=args.perform_function,
                                    )
                                )
                        elif (
                            detail_category_to_type(
                            ...
                            with get_openai_callback() as cb:
                                run_tool_answer, tool_call = (
                                    maintenance.filter_extracted_preference_mnp(
                                        incoming_preference=extracted_preference,
                                        existing_preferences=existing_preferences,
                                        perform_function=args.perform_function,
                                    )
                            ...
                            raise ValueError(
                                f"Detail category does not exist or mapping wrong"
                            )
                else:
                    tool_call = None
        ...
        filtered_data = filter(
            lambda d: "evaluation"
            in d["maintenance_questions"]["question_equal_extraction"],
            line["data"],
        )
