...
from utils.llm import get_embedding, get_llm_gpt4o
...
def parse_args():
    ...
    parser.add_argument(
        "--call_maintenance_function_dir",
        type=str,
        default="maintenance/evaluation/gpt4o/call_maintenance_function_for_eval/dataset/call_maintenance_function.jsonl",
    )
    parser.add_argument("--trace_by_langsmith", type=bool, default=False)
    parser.add_argument(
        "--langsmith_project_name", type=str, default="eval_of_maintenance"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="maintenance/evaluation/gpt4o/eval_of_maintenance/dataset/",
    )
...
def main():
    ...
    for line in track(train_dataset_lines):
        for conversation_data in tqdm(line["data"]):
            ...
            conversation_extraction = conversation_data[
            ...
            detail_category_type = detail_category_to_type(
                conversation_extraction["ground_truth_preference"]["detail_category"]
            )
            ...
            preference_equal_tool_call = preference_equal_dict["evaluation"][
                "maintenance_function_call"
            ]
            mean_existing_preferences.append(
                preference_equal_dict["evaluation"]["number_preferences_existing"]
            )

            preference_negate_dict = maintenance_questions["question_negate_extraction"]
            number_preferences_extracted_negate = preference_negate_dict[
                "number_preferences_negate_extracted"
            ]
            if number_preferences_extracted_negate == 1:
                preference_negate = preference_negate_dict[
                    f"extracted_preference_negate_0_full"
                ]
                mean_existing_preferences.append(
                    preference_equal_dict["evaluation"]["number_preferences_existing"]
                )
            ...
            preference_negate_tool_call = preference_negate_dict["evaluation"][
                "maintenance_function_call"
            ]

            preference_different_dict = maintenance_questions[
                "question_different_extraction"
            ]
            preference_different = preference_different_dict[
                "extracted_preference_different_full"
            ]
            preference_different_tool_call = preference_different_dict["evaluation"][
                "maintenance_function_call"
            ]
            mean_existing_preferences.append(
                preference_equal_dict["evaluation"]["number_preferences_existing"]
    ...
    mean_number_existing_preferences = sum(mean_existing_preferences) / len(
        mean_existing_preferences
    )
    ...
    mp_negate_accuracy = accuracy_score(
        y_true=mp_negate_true, y_pred=mp_label_negate_cleaned
    )

    mp_labels_different_true = [2 for i in range(number_mp)]
    mp_different_true = ["append_preference" for i in range(number_mp)]
    mp_different_accuracy = accuracy_score(
        y_true=mp_different_true, y_pred=mp_label_different
    )
    mp_different_custom_accuracy = custom_accuracy_different_mp(
        y_true=mp_different_true, y_pred=mp_label_different
    )
    ...
    if args.confusion_matrix:
        # create heatmap
        concat_label_mp_true = (
            mp_labels_equal_true + mp_labels_negate_true + mp_labels_different_true
        )
        concat_label_mp_pred = (
            mp_label_equal + mp_label_negate_cleaned + mp_label_different
        )
        ...
        for true_label, pred_label in zip(concat_label_mp_true, concat_label_mp_pred):
            ...
            col = predicted_label_mapping[
                pred_label
            ]  # Get the column index for the predicted label
        ...
        sns.heatmap(
            ...
            fmt="g",
            cmap="Blues",
            xticklabels=predicted_display_labels,
            yticklabels=true_display_labels,
        )
        ...
        plt.title(
            "Tally of Predicted Functions for Each Maintenance Query (mp)", fontsize=16
        )
        ...
        plt.yticks(fontsize=16)
        plt.title(
            "Normalized Tally of Predicted Functions for Each Maintenance Query (mp)",
            fontsize=16,
        )
    ...
    mnp_negate_accuracy = accuracy_score(
        y_true=mnp_negate_true, y_pred=mnp_label_negate_cleaned
    )

    mnp_different_true = ["update_preference" for i in range(number_mnp)]
    mnp_labels_different_true = [2 for i in range(number_mnp)]
    mnp_different_accuracy = accuracy_score(
        y_true=mnp_different_true, y_pred=mnp_label_different
    )
    ...
    if args.confusion_matrix:
        ...
        concat_label_mnp_true = (
            mnp_labels_equal_true + mnp_labels_negate_true + mnp_labels_different_true
        )
        concat_label_mnp_pred = (
            mnp_label_equal + mnp_label_negate_cleaned + mnp_label_different
        )
        ...
        sns.heatmap(
            ...
            fmt="g",
            cmap="Blues",
            xticklabels=predicted_display_labels_mnp,
            yticklabels=true_display_labels_mnp,
        )
        plt.xlabel("Predicted Labels", fontsize=16)
        plt.ylabel("True Labels", fontsize=16)
        plt.title(
            "Tally of Predicted Functions for Each Maintenance Query (mnp)", fontsize=16
        )
        plt.xticks(fontsize=16)
        plt.yticks(fontsize=16)
        ...
        tally_matrix_mnp_normalized = tally_matrix_mnp / tally_matrix_mnp.sum(
        ...
        plt.yticks(fontsize=16)
        plt.title(
            "Normalized Tally of Predicted Functions for Each Maintenance Query (mnp)",
            fontsize=16,
        )
        ...
        offset_positions = create_offset_positions(
            group_pos, len(accuracy_types), bar_width, space_between_bars
        )
        ...
        for i, accuracy in enumerate(accuracy_types):
            ...
            ax.bar(
                offset_positions[0, i],
                mp_accuracies[i],
                bar_width,
                label=accuracy if group_pos[0] == 0 else "",
                color=colors[i],
            )
            ax.text(
                offset_positions[0, i],
